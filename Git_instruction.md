...
Для того, чтобы исключить из отслеживания в репозитории определенные файлы или папки, необходимо создать новый файл с названием **`.gitignore`** из записать в него из названия или шаблоны, соответствующие таким файлам или папкам, например **``File1.jpg``**. 
...
## 9. Создание веток в Git с помощью команд
...
```
В результате создается новый указатель на текущий коммит. Важно помнить, что при создании ветки этой командой, вы остаетесь на ветке **master**.
...
Текущая ветка, на которой вы находитесь в данный момент, будет отмечена звездочкой : ***master***. Так же текущую ветку можно увидеть в интерфейсе VS Code в левом нижнем углу:

![show_branch](%D0%94%D0%971-2.jpg)

Но, как показывает практика, в очень редких случаях VS Code может не показать актуальную инфрмацию, поэтому рекомендуется проверять текущую ветку обоими способами.
...
## NN. Консольные команды по работе с папками и файлами в VS Code
...
ДЗ:
Какие варианты разрешения, указать коммит после слияния. Не дает удалить ветку, если не выполнили слияние.
Удаление веток - гит бранч -д + возможные ошибки

Создать аккаунт на гит.хаб.